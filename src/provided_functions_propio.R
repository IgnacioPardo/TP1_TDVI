...
preprocess_data <- function(var_to_predict, train_df, val_df,
  ...
  if (prop_noise > 0){
    ...
    numeric_keys <- c()
    for (key in setdiff(colnames(X_train), "obs_id_value")) {
        if (is.numeric(X_train[, key]) == TRUE) {
            numeric_keys <- c(numeric_keys, key)
        }
    }
    ...
    for (key in numeric_keys) {
        ...
        for (j in seq_len(nrow(X_train))) {
            ...
            if (p < prop_noise){
                min_val <- min(X_train[, key])
                max_val <- max(X_train[, key])
